...
<div align=center><img src="25.6IMG/总表.png"/></div>
...
自相关滞后是衡量时间序列中不同时间点之间相关性的统计指标。它表示当前时刻的值与过去某个时刻值之间的相关程度。

选择特证（经纬度），计算不同滞后的相关性，计算皮尔逊相关系数，得到自相关滞后列表。这样LLM就能理解时间序列的自相关特性，从而更好地进行预测。
